...
const OrderForm = ({ order, products, onSubmit, onCancel }: OrderFormProps) => (
  <form onSubmit={(e) => { e.preventDefault(); onSubmit(new FormData(e.currentTarget)); }} className="space-y-4">
    <div className="space-y-2 bg-slate-50 rounded-lg">
      ...
      {products.map((product) => (
        <div key={product.product_id} className="flex items-center space-x-2">
          <Input
            ...
            type="number"
            min="0"
            defaultValue={order?.products.find(p => p.productId === product.product_id)?.quantity || 0}
          />
          <Label htmlFor={`product-${product.product_id}`}>{product.name}</Label>
        </div>
      ))}
    </div>
    <div className="space-y-2">
