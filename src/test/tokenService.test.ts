...
import {
  ...
  setAuthCookies,
  getUserFromToken,
} from '@/app/api/services/allTokenService';
...
describe('Token Service Tests', () => {
  ...
  describe('Cookie Management Tests', () => {
    it('should set auth cookies', () => {
      ...
      const refreshTokenValue = refreshToken(testUserId, testRole);
      setAuthCookies(mockResponse, accessToken, refreshTokenValue);
  ...
  describe('Get User From Token Tests', () => {
    it('should extract user from token in request headers', async () => {
      const mockRequest = new Request('http://test.com', {
        headers: new Headers({
          'Cookie': 'accesstoken=valid.test.token'
        })
      }) as Request;

      const result = await getUserFromToken(mockRequest);
