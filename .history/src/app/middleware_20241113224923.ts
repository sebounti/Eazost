...
export default async function middleware(req: NextRequest) {
  ...
  if (req.nextUrl.pathname.startsWith('/api')) {
    ...
    if (refreshToken) {
		const { success: refreshSuccess, userId, account_type } = await verifySessionToken(refreshToken); // Utilisation du retour mis à jour
		if (refreshSuccess && userId !== undefined) {
		  console.log('Refresh token valide');
		  newAccessToken = generateAccessToken(userId, account_type); // Générer un nouveau token avec account_type
