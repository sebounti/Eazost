    ...
    if (refreshToken) {
      ...
      if (refreshSuccess && userId !== undefined) {
		...
		if (userId !== undefined && role !== undefined) {
			...
			console.error('userId or role is undefined');
			return NextResponse.redirect(new URL('/login', req.url)); // Redirigez vers la page de login en cas d'absence de `role`
		  }

        newAccessToken = generateAccessToken(userId, role);
        console.log('Nouveau token d’accès généré:', newAccessToken);

		//	Enregistrer le nouveau token d'accès dans les cookies
