      ...
      if (!success) {
		...
		if (refreshToken) {
			...
			if (refreshSuccess) {
				...
				const newAccessToken = generateAToken(userId, role);
				console.log('Nouveau token d’accès généré:', newAccessToken);

				// Envoie les nouveaux tokens en tant que cookies sécurisés
				setAuthCookies(NextResponse.next(), newAccessToken, refreshToken);
