...
import { generateToken } from '@/app/api/services/tokenService'; // Importation depuis tokenService.ts
...
import { getUserFromToken } from '@/app/api/services/tokenService'; // Utilisation de votre fonction existante
...
export async function POST(request: NextRequest) {
  try {
    ...
    const sessionId = await insertSession(user[0].users_id, token, ipAddress, userAgent);
  ...
  export async function PATCH(request: NextRequest) {
	try {
	  ...
	  if (error instanceof z.ZodError) {
		return NextResponse.json({ error: (error as z.ZodError).errors.map((e: { message: any; }) => e.message).join(", ") }, { status: 400 });
	  }
	  return NextResponse.json({ error: (error as Error).message }, { status: 500 });
	}
  }
