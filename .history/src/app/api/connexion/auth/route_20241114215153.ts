...
import { generateAccessToken, getUserFromToken } from '@/app/api/services/tokenService';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const token = generateAccessToken(user[0].users_id, user[0].account_type);
	...
	console.log("Cookie accesstoken défini avec la valeur :", response.cookies.get('accesstoken')?.value);

	console.log("Ajout du cookie dans les en-têtes de la réponse");
    console.log("Token généré:", token); // Affiche le token pour vérification    response.cookies.set('accesstoken', token, {
      httpOnly: true,
      secure: true,
      sameSite: 'strict',
      maxAge: 3600, // 1 heure
	...
	console.log("Cookie accesstoken défini avec la valeur :", token); // Vérifie la valeur du cookie

	localStorage.setItem('accessToken', token);
	console.log('Token accesstoken stocké dans localStorage:', localStorage.getItem('accessToken'));



    console.log("Cookie accesstoken défini avec la valeur :", response.cookies.get('accesstoken')?.value); // Nouvelle ligne
    return response;
