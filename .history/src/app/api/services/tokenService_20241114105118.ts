...
const verifyToken = (token: string | undefined): { userId: number, role: string } | null => {
    if (!token) {
...
describe('TokenService', () => {
  const SECRET_KEY = process.env.JWT_SECRET || 'votre_clé_secrète_de_test';

  test('doit valider un token JWT valide', () => {
    // Création d'un token de test
    const testPayload = { userId: 1, role: 'owner' };
  ...
  test('doit retourner null pour un token undefined', () => {
