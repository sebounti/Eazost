...
export async function verifyToken(token: string) {
    ...
    try {
        ...
        const decoded = jwt.verify(token, secretKey) as { userId: number, account_type: string }; // userId est un entier

        // Log du contenu décodé du token
        console.log('Token décodé dans verifyToken:', decoded);

        const userId = decoded.userId;
        const userAccount_type = decoded.account_type;

        console.log(`User ID: ${userId}, Role: ${userAccount_type}`);
        ...
        return { success: true, userId, account_type: userAccount_type };
    } catch (error) {
