...
export default async function middleware(req: NextRequest) {
  ...
  if (req.nextUrl.pathname.startsWith('/api')) {
    ...
    if (accessToken) {
      const { success, userId, account_type } = await verifySessionToken(accessToken); // Extraction de account_type
      ...
      if (success && userId !== undefined) {
        ...
        if (account_type !== requiredRole) { // Utilisation de account_type ici
    ...
    if (refreshToken) {
      const { success: refreshSuccess, userId, account_type } = await verifySessionToken(refreshToken); // Extraction de account_type
      if (refreshSuccess && userId !== undefined) {
        console.log('Refresh token valide');
        newAccessToken = generateAccessToken(userId, account_type); // Utilisation de account_type pour générer le nouveau token
