...
export default async function middleware(req: NextRequest) {
  ...
  if (req.nextUrl.pathname.startsWith('/api')) {
    ...
    if (accessToken) {
      const tokenResult = await verifySessionToken(accessToken);
      console.log('Résultat de la vérification du token :', tokenResult.success);

      if (tokenResult.success && tokenResult.userId !== undefined) {
        ...
        if (tokenResult.account_type && tokenResult.account_type !== requiredRole) {
    ...
    if (refreshToken) {
      const { success: refreshSuccess, userId, account_type } = await verifySessionToken(refreshToken);
