...
export default async function middleware(req: NextRequest) {
  ...
  if (req.nextUrl.pathname.startsWith('/api')) {
    ...
    if (accessToken) {
      const { success, userId, account_type } = await verifyToken(accessToken);
    ...
    if (refreshToken) {
      ...
      if (refreshSuccess) {
        ...
        if (userId !== undefined) {
          const newAccessToken = generateAccessToken(userId, account_type);
          console.log('Nouveau token d’accès généré:', newAccessToken);
        }
        console.log('Nouveau token d’accès généré:', AccessToken);
