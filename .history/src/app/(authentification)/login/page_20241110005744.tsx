	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
		  ...
		  if (response.ok) {
			...
			// Vérifiez si `err` est une instance de `Error` et affichez `err.message`.
			// Sinon, affichez une chaîne de caractères par défaut.
			if (err instanceof Error) {
			  setError(err.message);
			} else {
			  setError(String(err) || "Une erreur inconnue s'est produite.");
			}
		  }
