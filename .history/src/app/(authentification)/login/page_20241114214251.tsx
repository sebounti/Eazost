...
export default function LoginPage() {
	...
	// Utilisez le hook `useUser` pour accéder aux fonctions et données du contexte utilisateur
    ...
    try {
      ...
      if (response.ok) {
		...
		fetch('/api/verify-cookie')
  			...
  			.then(data => console.log('Token accesstoken depuis le serveur :', data.token));
  ...
  useEffect(() => {
	const token = document.cookie
	  ...
	  ?.split('=')[1];
	console.log('Token accesstoken récupéré depuis le client :', token);
  }, []);

  useEffect(() => {
	if (typeof window !== 'undefined') {
	  // Code qui utilise localStorage, par exemple :
	  const token = localStorage.getItem('accesstoken');
	  if (token) {
		// Traitement avec le token
	  }
	}
  }, []);
