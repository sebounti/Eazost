	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
		  ...
		  if (response.ok) {
			...
			if (data.user_id && data.user_name && data.user_role) {
		setCurrentUser({
			id	: data.user_id,
			name: data.user_name,
			role: data.user_role,
		...
		console.log("Utilisateur actuel :", currentUser);
			...
			alert("La connexion a réussi. Vérifiez les log pour voir les données.");
...
function setCurrentUser(arg0: { id: any; name: any; role: any; }) {
