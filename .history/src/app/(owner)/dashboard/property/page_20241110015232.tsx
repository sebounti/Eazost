...
import { UserContext } from "@/context/userContext"; // Par exemple
import { useContext } from "react";

export default function LogementsPage() {
  const userContext = useContext(UserContext);
  const currentUser = userContext?.currentUser;
  const users_id = currentUser?.id;
  ...
  return (
    <div className="container p-4 mx-auto">
      <h1 className="text-5xl font-extrabold tracking-tight p-2 text-center">Gestion des Logements</h1>
      <p className="mb-8 text-muted-foreground p-2 text-center">Gérez vos propriétés, codes d&apos;accès, Carte info logement  et commandes en un seul endroit</p>

      {/* Bouton pour ouvrir le modal */}
      <button onClick={() => setIsDialogOpen(true)} className="bg-amber-400 rounded-xl shadow-lg p-3">Ajouter Logement</button>
      ...
      {isDialogOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70">
          <div className="bg-white p-4 border-gray-400  border-2 rounded-xl shadow-lg max-w-lg w-full max-h-[90vh] overflow-y-auto ">
            <button onClick={() => setIsDialogOpen(false)} className="text-gray-400 mb-2 text-lg "><MdClose />
			</button>
