...
// Configuration des mocks avec les types appropriés
jest.mock('../db/db', () => ({
	db: {
	  insert: jest.fn().mockReturnValue({
		values: jest.fn().mockResolvedValue({}) // Simuler une insertion réussie
	  }),
	  select: jest.fn().mockResolvedValue([
		{ users_id: testUserId, token: validToken, used: false } // Renvoie un token valide
	  ]),
	  update: jest.fn().mockResolvedValue({}) // Simule une mise à jour réussie
	}
  }));
