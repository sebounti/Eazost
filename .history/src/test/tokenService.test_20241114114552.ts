...
import { generateAccessToken, saveTokenToDatabase, verifyAccountToken, verifyToken } from '@/app/api/services/tokenService';
...
jest.mock('@/db/db', () => ({
  db: {
    insert: jest.fn().mockResolvedValue(true), // Mock de l'insertion réussie
    select: jest.fn().mockResolvedValue([{ users_id: testUserId, token: validToken, used: false }]), // Mock de la sélection
    update: jest.fn().mockResolvedValue(true), // Mock de la mise à jour réussie
  },
}));
...
describe('Token Service', () => {
  ...
  it('devrait envoyer les tokens en tant que cookies', () => {
    ...
    const response = { headers: { append: jest.fn() } };
...
function setAuthCookies(response: { headers: { append: jest.Mock<any, any, any>; }; }, accessToken: string, refreshToken: string) {
	response.headers.append('Set-Cookie', `accessToken=${accessToken}; Path=/; HttpOnly; SameSite=Strict; Max-Age=900`);
	response.headers.append('Set-Cookie', `refreshToken=${refreshToken}; Path=/; HttpOnly; SameSite=Strict; Max-Age=604800`);
}
function verifySessionToken(validToken: string) {
...
async function verifySessionToken(token: string) {
	try {
		...
		const session = await db.select().from('sessions').where({ token, used: false }).first();
		if (!session) {
