...
jest.mock('@/db/db', () => ({
  ...
  select: jest.fn().mockResolvedValue([{ users_id: testUserId, token: validToken, used: false }]),  // Retourne un token valide dans la base de données
...
describe('Token Service', () => {
  ...
  it('devrait envoyer les tokens en tant que cookies', () => {
    const response = { headers: { append: jest.fn() } };  // Simuler l'objet réponse
