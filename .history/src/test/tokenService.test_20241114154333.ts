...
describe('Token Service Tests', () => {
  ...
  it('should include specific permissions in the token payload', () => {
	...
	const token = jwt.sign({ userId: testUserId, role: testRole, permissions }, secretKey, { expiresIn: '1h' });
	const decoded = jwt.verify(token, secretKey) as JwtPayload & { permissions: string[] };
