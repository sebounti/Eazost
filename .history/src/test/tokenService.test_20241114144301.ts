...
describe('Token Service Tests', () => {
  ...
  beforeAll(() => {
    ...
    // Générer un token expiré (expire dans 1 seconde)
  ...
  it('should generate a valid access token', () => {
    const token = validAccessToken; // Utilisation du token valide
  ...
  describe('Token Error Handling Tests', () => {
    it('should reject an expired access token', async () => {
      ...
      expect(result.message).toBe('Unknown error'); // Vérifie que le message d'erreur correspond à l'attendu
    });
    ...
    it('should reject an invalid access token', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.message).toBe('Invalid token'); // Vérifie que le message d'erreur correspond à l'attendu
    });
    ...
    it('should reject an expired refresh token', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.message).toBe('Refresh token expired'); // Vérifie que le message d'erreur correspond à l'attendu
    });
