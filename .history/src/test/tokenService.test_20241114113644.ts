...
jest.mock('@/db/db', () => ({
  insert: jest.fn().mockReturnValue({
    values: jest.fn().mockResolvedValue({}),  // Remplacé par un objet vide, car Drizzle ne s'attend pas à un booléen
  }),
  select: jest.fn().mockResolvedValue([{ users_id: testUserId, token: validToken, used: false }]),  // Valeur d'exemple pour le token
  update: jest.fn().mockResolvedValue({}),  // Remplacé par un objet vide, pour correspondre à la structure attendue par Drizzle
}));
