...
jest.mock('../db/db', () => ({
  db: {
    insert: jest.fn().mockReturnValue({
      values: jest.fn().mockResolvedValue<DrizzleValues>({}),
    }),
    select: jest.fn().mockResolvedValue<DrizzleValues[]>([
      { users_id: testUserId, token: validToken, used: false }
    ]),
    update: jest.fn().mockResolvedValue<DrizzleValues>({})
  }
}));
