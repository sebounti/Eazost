...
export const UserProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
	  ...
	  if (storedUser) {
		// Vérifier si l'utilisateur est connecté et a un type de compte
		const parsedeUser = JSON.parse(storedUser);
			if (parsedeUser.isLogged && parsedeUser.account_type) {
  setCurrentUser(parsedeUser);
		...
		// Rediriger vers la page de connexion si pas utilisateur stocké.
		router.push('/login');
	  }
	}
  }, [router]);
