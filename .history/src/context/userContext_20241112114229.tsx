...
export const UserProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const checkAuth = async () => {
	if (typeof window !== "undefined") {
	  ...
	  console.log("Token récupéré dans checkAuth :", token); // Log pour vérifier le token

	  if (token) {
		const response = await fetch('/api/tokenservice', {
		  headers: {
			'Authorization': `Bearer ${token}`, // Envoi du token en tant que Bearer token si nécessaire
		  },
		});
		console.log("Réponse de tokenservice :", response);
